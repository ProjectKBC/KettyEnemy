...
public class PlayerTest : MonoBehaviour {
  ...
  private void Attak () {
    if (Input.GetKey (KeyCode.X)) {
      GameObject bullet = (GameObject) Instantiate (normalBullet);
      bullet.transform.position = this.transform.position;
      bullets.Add(bullet);
    }
    for (int i = 0; i < bullets.Count; i ++) {
      bullets [i].transform.position = new Vector3 (bullets[i].transform.position.x, bullets [i].transform.position.y, bullets [i].transform.position.z - 0.1f);
    }
  }
  */
